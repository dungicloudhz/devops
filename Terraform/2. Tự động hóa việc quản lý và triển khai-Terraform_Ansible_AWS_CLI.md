...
# 11. Cấu hình Terraform Provisioners để quản lý cấu hình Ansible

Mục đích bài học: Bootstrapping cơ sở hạ tầng qua Terraform Provisioners
...
2. Nếu một provisioners thất bại khi chạy cho một tài nguyên trong Terraform, tài nguyên đó được đánh dấu là tainted (bị nhiễm).
`Một tài nguyên bị đánh dấu là tainted sẽ bị xóa khỏi (destroyed) trong lần tiếp theo thực hiện lệnh terraform apply và tạo lại tài nguyên`.
...
**Ví dụ sử dụng Provisioners bên trong một Terraform Resource**
```yaml
resource "aws_instance" "jenkins-worker-cali" {
... 
}
...
---
- host: "{{ passed_in_hosts }}" #
  become: yes
  remote_user: ec2-user
  become_user: root # lấy quyền của user-root
  tasks:
...
cd ..
vim ansible.cfg
# Tại phần mặc định defaults thêm một vài dòng cấu hình mặc định như sau
inventory = ./ansible_template/inventory_aws/tf_aws_ec2.yml # chỉ ra path tới dynamic inventory được cung cấp bởi ansible -> sẽ cho ansible biết các cấu hình nào được truy vấn cách tổng hợp các tham số được truy vấn, của aws ec2 được trả về để chúng ta có thể truyền vào các tên máy chú trong các ansible play book
enable_plugins = aws_ec2 # bật plugin,plugin sẽ giúp cho chúng ta có thể truy vấn địa chỉ dynamic inventory của aws 
interpreter_python = auto_silent # cảnh bảo im lặng warning mà ansible có thể hiển thị về cái trình thông dịch python trong cac ec2 remote

# Tạo file dynamic inventory
cd ansible_templates/
mkdir inventory_aws
cd inventory_aws/
wget https://raw.githubusercontent.com/phuongluuho/TerraformAnsible/main/tf_aws_ec2.yml
...
# Trước khi chúng ta thêm một đọan terraform code chúng ta cần cài đặt
pip3.11 install botto3 --user # sau bước này chúng ta sẽ thực hiện chuyển đổi plugin

cd ..
vim instances.tf
# Thêm provisioner vào resource đã tạo ở trên jenkins-master
resource "aws_instance" "jenkins-master" {
    provisioner "local-exec" {
        command = <<EOF aws --profile ${var.profile} ec2 wait instance-status-ok --region ${var.region-master} --instance-ids ${self.id} ansible-playbook --extra-vars 'passed_in_hosts=tag_Name_${self.tags.Name}' ansible_templates/jenkins-master-sample.yml EOF
    }
}`

# Thêm provisioner vào resource đã tạo ở trên jenkins-worker
resource "aws_instance" "jenkins-worker-cali" {
    provisioner "local-exec" {
        command = <<EOF aws --profile ${var.profile} ec2 wait instance-status-ok --region ${var.region-worker} --instance-ids ${self.id} ansible-playbook --extra-vars 'passed_in_hosts=tag_Name_${self.tags.Name}' ansible_templates/jenkins-worker-sample.yml EOF
    }
}`
...
terraform plan
terraform apply
# Đăng nhập máy ảo ec2 vào kiểm tra các tài nguyên đã được cài đặt chưa
```